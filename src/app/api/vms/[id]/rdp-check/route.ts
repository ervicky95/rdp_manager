...
export async function POST(
  ...
  try {
    ...
    await authRepo.getDb()
      .prepare(
        `INSERT INTO audit_logs (id, vm_id, event_type, severity, message, metadata, created_at)
         VALUES (?, ?, 'command_rdp_check', 'info', ?, ?, ?)`
      )
      .bind(crypto.randomUUID(), id, `RDP check command queued for "${vm.name}"`, JSON.stringify({ command_id: command.id }), new Date().toISOString())
      .run();

    return NextResponse.json({
      command,
      message: 'RDP check command queued. Agent will execute on next poll.',
    });
  } catch (error) {
    ...
    console.error('POST /api/vms/[id]/rdp-check error:', error);
    const message = error instanceof Error ? error.message : 'Internal error';
    if (message.includes('Invalid command')) {
      return NextResponse.json({ error: message }, { status: 400 });
    }
    return NextResponse.json({ error: 'Failed to queue RDP check command' }, { status: 500 });
  }
}